...
export default function SignInPage() {
  ...
  const onSubmit: SubmitHandler<SignInFormValues> = async (data) => {
    ...
    try {
      // Step 1: Find user by phone number to get their email
      const userWithPhone = await getUserByPhone(data.phone);
      
      if (!userWithPhone || !userWithPhone.email) {
          throw new Error('USER_NOT_FOUND_BY_PHONE');
      }

      // Step 2: Sign in with the retrieved email and provided password
      const userCredential = await signInWithEmailAndPassword(auth, userWithPhone.email, data.password);
      ...
      if (error.message === 'USER_NOT_FOUND_BY_PHONE') {
  ...
  const handleForgotPassword = async (e: React.MouseEvent) => {
    ...
    setIsLoading(true);
    try {
        const userWithPhone = await getUserByPhone(phone);
        if (!userWithPhone || !userWithPhone.email) {
        ...
        if (error.message === 'USER_NOT_FOUND') {
            message = "لم يتم العثور على حساب مرتبط برقم الهاتف هذا.";
        }
        toast({
